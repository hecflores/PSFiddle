		...
		elseif(-not ($value -match ('[`'+$variablePrefix+'][(]([^)\`'+$variablePrefix+']*?)[)]'))){
			...
			while($oldValue -ne $value -and $value -match ('[`'+$variablePrefix+'][(]([^)\`'+$variablePrefix+']*?)[)]') -and -not $this.Context().IsParmeterizingDisabled()){
				...
				try{
					...
					$this.Error("Failed to parameterize $($originalValue): $($_.Exception.Message)")
					$value = $null
...
class UIParameterCollection : HasCollectionContext {
    ...
    [String] FillParameterizedString([String] $inputString, [UIParameterCollection] $parameters){
		#TEMP $this.Action("{white}Filling Parameters{gray} for Input '{magenta}$($inputString){gray}' ")
		$this.PushIndent()
        $finalString = $inputString
        foreach($parameter in $parameters.Items()){
			#TEMP $this.Action("{white}Filling Parameter{gray} {magenta}'$($parameter.ParameterName())'{gray}")
			$this.PushIndent()
            $thisPattern = '[$][{(]'+$parameter.ParameterName()+'[})]'
            $foundParameter = $this.Get($parameter.ParameterName())
			
			#TEMP $this.Action("{white}Filling Parameter{gray} {magenta}'$($parameter.ParameterName())'{gray}")
			if(-not $foundParameter){
				$this.Error("Parameters", "The input value for {white}$($parameter.ParameterName()){gray} came back with nothing... Something went wrong with resolving the input value")
				$this.PopIndent()
				continue
			}
			
			$foundParameter.IsRequired($true)

			if(-not $foundParameter.Value()){
				$this.Context().Action("{yellow} Value not defined{gray}, The input value for $($parameter.ParameterName()) came back with nothing... Something went wrong with resolving the input value")
				$this.PopIndent()
				continue
			}

			
            # This section basicly converts the input value to the object its for (According to the data type) then to a code use form
            [UIParameter] $parameter = $foundParameter
            $original___Input = $parameter.Value()
			$lastValue = $null
			$replaceValue = $true
			...
			if($replaceValue){
				...
				$parameterize = [UIParameterCollection]::FromParameterizedString($this.Context(), $original___Input, $this.CurrentScope())	
				# $parameterize.Items() | Foreach {Write-Host $_.ParameterName()}
				# Write-Host "$($foundParameter.CurrentScope().Name()) -eq $($this.CurrentScope().Name())"
				if($foundParameter.CurrentScope().Name() -eq $this.CurrentScope().Name()){
					
					if($parameterize.Get($foundParameter.ParameterName(), $false)){
						
						if(-not  $this.CurrentScope().ParentScope()){
							$this.Error("Found recursive parameter setting '$($foundParameter.ParameterName())' and there is no parent scope to grab from to resolve the recursion")
						}
						else{
							# Write-Host "Found Parameter: $($original___Input)"
							$original___Input = $this.CurrentScope().ParentScope().Parameters().FillParameterizedString($original___Input,$parameterize)
						}
					}
				}
				
				$transformedInput = $parameter.ParameterType().Definition().TransformInputValue($original___Input)
				$stringifiedInput = $parameter.ParameterType().Definition().TransformParameterToCodeUse($transformedInput)

				# Replace the parameter
				$finalString=$finalString -replace $thisPattern, $stringifiedInput
				
				# Write-Host "Parameter: $($finalString)"
				$parameterize = [UIParameterCollection]::FromParameterizedString($this.Context(), $finalString, $this.CurrentScope())	
				if($parameterize.Items() | Where {$_} | Where {$_.ParameterName() -eq $foundParameter.ParameterName()}){
					if($foundParameter.CurrentScope().ParentScope()){
						$finalString = $foundParameter.CurrentScope().ParentScope().Parameters().FillParameterizedString($finalString,$parameterize)
					}
				}
